...
//this  is not done yet
  addplayer(PlayerName) { //when it returns none, algo should try to add a different player- maybe with the needs of the team?
    ...
    if (this.isteamfull()){ //if the team is full, ERROR. //true exits the loop
      ...
      { //only add player if they can fill a role not already filled, meaning   they didnt just queue for one role
        ...
        //   }
          ...
          if (deniedroles==PlayerName.role.length){
            ...
            //this is all literally just return this.addplayercheck()
            // result=this.addplayercheck()
            // if (result = true) {
            //   return true //team is full, player add successful, exit loop
            // } else if (result = false) {
            //   return false //there wasan error, kick a random player and try again
            // } else if (result = null) { //team not full, roll again
            //   return null 
            // } else {
            //   //add check to make sure the result is of type player (typeof), or use instance of?
            //   return result //this means that we need to remove a player and role again, this player is in result.
            // }
            return this.addplayercheck()
            //true means player add successful, false means unsuccessful kick a random player add a new player, null means simply add a new player, else means kick the returned player, and add a new one
  ...
  addplayercheck() { //true = sucessfully made team, null = need more players, false means error, find new player
      if (this.isteamfull()==true){ //if the team is full
          ...
          if (result==true){ // ...if every role is accounted for
              ...
              if (!result!=null){//(result!=null && result !=true) { //meaning the algo returned a player value. also mean swe weren't able to balance roles with current team and will need to pick again
              ...
              console.log("we are missing a {result[1]}! kick a player") 
  ...
  //what if we didnt add the player if they didnt fit the role int he first place
  ...
  balanceroles2() {
        ...
        let arr2=this.filledroles
          ...
          if(player.queuedroles.length=1){ //if player only has one role left, lock that role and re-run balance roles 2 just in case
            ...
            if(player.queuedroles[0]==null)
            {
              ...
              return player; //this means we have a player who can not fit into the team. balance roles was unsucessful //return player so that way they can be kicked
            }
            switch (role) {
              case "Tank":
    ...
    return null //it was a success. should be true
  }
...
//true means player add successful, false means unsuccessful kick a random player add a new player, null means simply add a new player, else means kick the returned player, and add a new one
function create_team(teamid, size=5, roleblock=true, maxteams,rerollamount=0,team=null){  // you have to put a team id in!
  console.log("player pool1:"+playerpool)
  //console.log(playerpool)
  console.log("playerpool2: "+playerpool2)
  if(team==null) {
    var team = new Team(teamid, size, roleblock)
  } else if (rerollamount>0) {
    var team=team //a team should be passed
  } else {
    console.log("we shouldnt reach here :(")
    throw new console.error("team should not be null or reroll should be 1 at least");
  }
    for (let i = 0; i < size; i++){ //allows dynamic change of team size in event of player kick
      ...
      outcome = team.addplayer(player)  //outcome will be true upon success or false upon error team full, null for player denied->request new one
      
      
        //PROBLEM:
      //wheni call the create team function from reroll++, it creates a team at the id, effectively deleting the team. now bueno!, should id elete team and make new? or should i create and if function 
      if(outcome==true) {
        console.log("outcome is true, team full, add successful")
        break; //team is full and player add was successul
      } 
      else if(outcome==false) { //there was an error, kick a random player and add a player again (enter while loop)
        console.log("outcome is false, deleting random player, tryin again!")
        playertemp=team.players.pop() //delete player, put back into player pool
        playerpool2.push(playertemp)
        //make player more random
        break;
      } 
      else if (outcome==null) { //if null, enter the while loop below to repeat add player
       ...
       while (outcome == null){ //keeps trying to add a player until it is successful (until outcomr isnt null)
          if (outcome != null){  // if the outcome wasnt none to begin with, break out of this loop
            break
          } //this may need to be further elaborated on
      ...
      } else { //else , result is gonna be a player. kick the returned player and brea
        console.log("outcome returned result: "+outcome+", deleting player")
        playertemp=outcome
        team.players= team.players.filter(a => a !== playertemp); //deletes player
        playerpool2.push(playertemp) //add player back to pool

        //we should make a function when we remove a player, it removes their roles from team
      }
    }
    ...
    if(team.players.length<size && playerpool.length != 0){ //if team is not full, and we have players to choose from. we need to re-roll
      if(rerollamount>0) { //if this is already our second role push a player anyway, force push a player
        let player = choose_random_player() // save random player (because they are removed now)
        if(rerollamount>3) {
          console.log('theres a probelm if we reach here! we shouldnt have to roll this many times!')
          return false
        }
        result=team.force_addplayer(player)
        console.log("force adding player "+player.name+" result: "+result)
        rerollamount++
        create_team(teamid,size,roleblock,maxteams,rerollamount,team)
