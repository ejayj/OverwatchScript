        ...
        //   }
          ...
          if (deniedroles==PlayerName.role.length){
           ...
           // make sure every role is accounted for. 
            ...
            //if it is not true and not null, -> if it is false do x, if it is not false, (meaning a player), do y)
...
function create_team(teamid, size=5, roleblock=true, maxteams,rerollamount=0){  // you have to put a team id in!
  ...
  console.log("playerpool2: "+playerpool2)
    ...
    for (let i = 0; i < size; i++){ //allows dynamic change of team size in event of player kick
      ...
      if(outcome==true) {
        break; //team is fll and player add was successul
      }

      if(outcome==false) { //there was an error, kick a random player and add a player again (enter while loop)
        break;
      }

      //if null, enter the while loop below to repeat add player

      //else , result is gonna be a player. kick the returned player and brea
